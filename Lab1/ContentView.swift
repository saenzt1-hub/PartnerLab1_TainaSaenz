...
struct ContentView: View {
    let dogNames = [
        "Airedale Terrier", "American Foxhound", "Dutch Shepherd", "Havanese", "Leonberger", "Mudi", "Norwegian Lundehund", "Pharaoh Hound", "Scottish Terrier", "Tosa"
    ]
    ...
    var dogsDict: [String: String] {
        Dictionary(uniqueKeysWithValues: zip(dogNames, dogDescription))
    }
    
    @State private var dog: String? = nil
    ...
    var body: some View {
        VStack(spacing: 8) {
            Text("Tap on a dog to see its description")
                .font(.title2)
                .fontWeight(.medium)
                .padding(.top, 20)
            
            LazyVGrid(columns: columns, spacing: 8) {
                ForEach(dogNames.prefix(9), id: \.self) { dog in
                    dogImage(dog: dog)
                }
            }
    ...
    func dogImage(dog: String) -> some View {
        Image(dog)
            .resizable()
            .scaledToFit()
